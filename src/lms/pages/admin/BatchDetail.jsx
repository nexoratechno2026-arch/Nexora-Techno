...
import { collection, query, where, getDocs, getDoc, orderBy, addDoc, updateDoc, deleteDoc, doc, serverTimestamp } from "firebase/firestore";
...
export default function BatchDetail() {
  ...
  async function fetchAll() {
    ...
    try {
      ...
      const mappedMembers = memRes.docs.map(d => {
        const data = d.data();
      ...
      const modList = modRes.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0));
      setModules(modList);

      if (modList.length) {
        let tasksList = [];
        for (const m of modList) {
          const tasksQ = query(collection(db, "tasks"), where("module_id", "==", m.id));
          const tRes = await getDocs(tasksQ);
          tasksList.push(...tRes.docs.map(d => ({ id: d.id, ...d.data() })));
        }
        setTasks(tasksList);
      }
    } catch (e) {
  ...
  async function saveModule(e) {
    ...
    try {
      if (editModule) {
        ...
        await addDoc(collection(db, "modules"), { ...payload, created_at: serverTimestamp() });
  ...
  async function addIntern(userId) {
    try {
      await addDoc(collection(db, "batch_members"), { batch_id: batchId, user_id: userId, created_at: serverTimestamp() });
  ...
  if (!batch) {
    return <LMSLayout><div className="p-6 text-slate-500">Batch not found.</div></LMSLayout>;
  }
  ...
  return (
    <LMSLayout>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        {/* Back + Header */}
        <div>
          <Link to="/lms/admin/batches" className="inline-flex items-center gap-1 text-slate-500 hover:text-slate-800 text-sm mb-4 transition-colors">
                              ...
                              return (
                                <div key={task.id} className="flex items-center gap-3 p-3 rounded-lg bg-white/3 border border-slate-200 group">
                                  ...
                                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Link to={`/lms/admin/tasks/${task.id}/submissions`} className="text-xs text-slate-500 hover:text-sky-400 px-2 py-1 rounded-md hover:bg-sky-500/10 transition-all">Reviews</Link>
                      ...
                      {members.map((m) => (
                        <tr key={m.user_id} className="hover:bg-white/2 transition-colors">
                          <td className="px-5 py-3">
                            <Link to={`/lms/admin/interns/${m.user_id}`} className="text-slate-700 hover:text-sky-400 font-medium transition-colors">
                              {m.profiles?.name || "—"}
                            </Link>
                          </td>
