  ...
  async function fetchData() {
    ...
    try {
      ...
      const subsRaw = subRes.docs.map(d => ({ id: d.id, ...d.data() }))
        .sort((a, b) => new Date(a.submitted_at?.toDate() || 0) - new Date(b.submitted_at?.toDate() || 0));
      ...
      <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
        ...
        <div>
          {task?.modules?.batch_id && (
            <Link to={`/lms/admin/batches/${task.modules.batch_id}`} className="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-800 text-sm font-medium mb-6 transition-colors">
