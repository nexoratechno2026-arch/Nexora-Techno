...
function AdminInterns() {
  ...
  const fetchAllData = async () => {
    try {
      setLoading(true);
      const [internsRes, appsRes, contactsRes] = await Promise.all([
        getDocs(collection(db, "interns")),
        getDocs(collection(db, "internship_applications")),
        getDocs(collection(db, "contact_messages"))
      ]);
      setInternList(internsRes.docs.map(d => ({ fb_id: d.id, ...d.data() })));
      setApplicationList(appsRes.docs.map(d => ({ fb_id: d.id, ...d.data() })));
      setContactList(contactsRes.docs.map(d => ({ fb_id: d.id, ...d.data() })));
    } catch (err) {
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await addDoc(collection(db, "interns"), newIntern);
  ...
  const handleUpdateAllDurations = async () => {
    ...
    try {
      const batchSize = 400;
      for (let i = 0; i < internList.length; i += batchSize) {
        const chunk = internList.slice(i, i + batchSize);
        const batch = writeBatch(db);
        chunk.forEach(intern => {
          const docRef = doc(db, "interns", intern.fb_id);
          batch.update(docRef, {
            duration: "05/05/2026 - 25/05/2026",
            completionDate: "2026-05-25",
            status: "Completed",
      ...
      setMessage(`✅ Updated ${internList.length} interns to 20 days`);
      fetchAllData();
    } catch (err) {
